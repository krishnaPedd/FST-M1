...
import static org.hamcrest.CoreMatchers.equalTo;
    ...
    public class Activity2 {
        ...
        public void getUserInfo() {
            ...
            Response response =
                    given().contentType(ContentType.JSON) // Set headers
                            .pathParam("username", "krishna") // Pass request body from file
            ...
            response.then().body("username", equalTo("krishna"));
            response.then().body("firstName", equalTo("krishna"));
            response.then().body("lastName", equalTo("p"));
            response.then().body("email", equalTo("krishnap@mail.com"));
        ...
        public void deleteUser() throws IOException {
            Response response =
                    given().contentType(ContentType.JSON) // Set headers
                            .pathParam("username", "krishna") // Add path parameter
                            .when().delete(ROOT_URI + "/{username}"); // Send POST request

            // Assertion
            response.then().body("code", equalTo(200));
            response.then().body("message", equalTo("krishna"));
        }
    }
